...
//		e.printStackTrace();
	...
	private static void uruchomMenu() throws SQLException {
		
		Connection polaczenie = OBD_namiary.sprawdzPolaczenie();
		System.out.println("......................");
		...
		while (!zakoncz) {
			...
			case "2":
				try {
				OBD_tworzenie_db.tworzenie(polaczenie);
				System.out.println(menu);
				} catch (Exception e) {
				}
				break;
			case "3":
				try {
				OBD_usuwanie_db.usuwanie(polaczenie);
				System.out.println(menu);
				} catch (Exception e) {
				}
				break;
			case "4":
				try {
				OBD_wyswietlanie_db.wyswietlanie(polaczenie);
				System.out.println(menu);
				} catch (Exception e) {
	...
	private static void ocenianie(Connection polaczenie, Scanner scn) {
		int idp, idn, ido, idu;
		String ocena;
		try {
			...
			OBD_ocenianie.wystawOcene(polaczenie, idp, idn, ido, idu, ocena);
